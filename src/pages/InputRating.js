...
export class InputRating extends Component {
  ...
  componentDidMount = () => {
    let map = this.props.rating.data_unreview.map(function (item) {
      return {
    ...
    return (
      <View style={{flex: 1, backgroundColor: 'white'}}>
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          {this.props.rating.data_unreview.map((item, index) => (
            <View key={index}>
              {item.product ? (
                <>
                  <Card
                  ...
                  {this.props.rating.data_unreview.length > 1 && (
                    <Card.Divider
                      style={{
                        borderWidth: wp('0.1%'),
                        marginTop: wp('3%'),
