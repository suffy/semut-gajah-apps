...
export class FormInputVerifikasiOTPSMSOld extends Component {
  _isMounted = false;
  constructor(props) {
    super(props);
  ...
  cekOTP = async () => {
    ...
    try {
      ...
      if (data !== '' && data['success'] == true) {
        ...
        await axios
          .post(`${CONFIG.BASE_URL}/api/auth/erp/register`, formData, {
            headers: {
  ...
  componentDidUpdate() {
    if (this.state.timer === 0) {
      ...
      this.setState({buttonTimbul: true});
    }
  }

  getCloseAlertModal() {
    this.setState({modalAlert: !this.state.modalAlert, tambahan: ''});
    if (
      ...
      this.props.navigation.replace('Login');
    } else if (this.state.alertData == 'Akun anda sudah terdaftar') {
      this.props.navigation.replace('Login');
    } else if (
      this.state.alertData ==
      'Nomor telepon sudah digunakan, silahkan menggunakan nomor telepon lainnya'
    ) {
      this.props.navigation.replace('RegisterUserLama');
    }
  }
  ...
  render() {
    const {timer, loadingApi, buttonTimbul} = this.state;
    return (
      <ScrollView showsVerticalScrollIndicator={false} style={styles.scroll}>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.posision}>
            ...
            <IconOTP style={styles.icon} width={wp('6%')} height={hp('6%')} />
            <TouchableOpacity
